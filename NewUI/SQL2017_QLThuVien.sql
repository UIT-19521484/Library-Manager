...
@TenSach NVARCHAR(100), @TacGia NVARCHAR(100), @NhaXuatBan NVARCHAR(200), @TenTL NVARCHAR(100), @TonTai INT, @DaMuon INT
AS
BEGIN
	DECLARE @MaTL INT
	SELECT @MaTL = MaTL FROM THELOAI
	WHERE TenTL = @TenTL
	INSERT INTO SACH(TenSach, TacGia, NhaXuatBan, TonTai,DaMuon, MaTL) VALUES (@TenSach, @TacGia, @NhaXuatBan, @TonTai, @DaMuon, @MaTL)
END
GO
